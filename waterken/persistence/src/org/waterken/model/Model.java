...
Model {
     ...
     * model at any time. An invocation from another thread must block until
     * the model becomes available. A recursive invocation from the same thread
     * must cause an error.</p>
     * <p>
     * If {@link Transaction#run invocation} of the <code>body</code> causes an
     * {@link Error}, all modifications to objects in the model must be
     ...
     * existing state was modified. If state was modified, the transaction
     * must be aborted, in the same manner described previously.
     * {@link Root#store Storing} a new {@link Root root} binding is
     * considered a modification.</p>
     * <p>
     ...
     * {@link Root#heap persistent address space}.</p>
     * <p>
     * Only if the current transaction commits will the
     * {@link Root#effect enqueued} {@link Effect}s be
     * {@link Transaction#run executed}; otherwise, they will be discarded. The
     * effects will be executed in the same order as they were enqueued.
     * Effects from a subsequent transaction will not be executed until all
     * effects from the current transaction have been executed. An
     * {@link Effect} must not access objects in the model, but may schedule
